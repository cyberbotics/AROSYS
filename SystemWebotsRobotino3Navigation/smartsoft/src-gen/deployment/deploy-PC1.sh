...
DEPLOYMENT_DIRECTORY=SystemWebotsRobotino3Navigation.deployment/
...
###############################
echo "Sourcing pre-deployment script for ComponentWebotsRobotino3... (errors might be ignored)"
DEPLOY_LIBRARIES=""
DEPLOY_COMPONENT_FILES=""
source src/predeploy-ComponentWebotsRobotino3.sh 2>&1
...
DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3=""
for I in $DEPLOY_COMPONENT_FILES; do
	if ls $REFERENCED_PROJECT_ComponentWebotsRobotino3/$I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3="$DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3 $REFERENCED_PROJECT_ComponentWebotsRobotino3/$I"
	elif ls $I > /dev/null 2>&1; then
		DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3="$DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3 $I"
	fi
done
...
## BEHAVIOR FILES
shopt -u | grep -q nullglob && changed=true && shopt -s nullglob
for entry in "$REFERENCED_PROJECT_ComponentWebotsRobotino3"/model/*.smartTcl
do
  DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_ComponentWebotsRobotino3="$DEPLOY_COMPONENT_TCL_MODEL_FILES_ComponentWebotsRobotino3 $entry"
done
[ $changed ] && shopt -u nullglob; unset changed

echo "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_ComponentWebotsRobotino3 "
#########################
...
$SMART_ROOT_ACE/lib/libCommBasicObjects.so*
src/ComponentWebotsRobotino3_data
src/startstop-hooks-ComponentWebotsRobotino3.sh
$SMART_ROOT_ACE/bin/ComponentWebotsRobotino3
src-gen/combined-ini-files/ComponentWebotsRobotino3.ini
$SMART_ROOT_ACE/lib/libCommBasicObjects.so*
...
#rsync -l -r -v --progress --exclude ".svn" $DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3 $TMPDIR/ComponentWebotsRobotino3_data/
if [ ! "$DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_FILES_PATHS_ComponentWebotsRobotino3 $TMPDIR/ComponentWebotsRobotino3_data/ 2>&1
fi

if [ ! "$DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_ComponentWebotsRobotino3" = "" ]; then
	cp -rv $DEPLOY_COMPONENT_BEHAVIOR_MODEL_FILES_ComponentWebotsRobotino3 $TMPDIR/behaviorFiles/ 2>&1
fi

cp -v $REFERENCED_PROJECT_ComponentWebotsRobotino3/smartsoft/src/startstop-hooks.sh $TMPDIR/startstop-hooks-component-ComponentWebotsRobotino3.sh 2>/dev/null
