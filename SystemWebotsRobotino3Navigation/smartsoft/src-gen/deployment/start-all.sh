...
start)
	...
	xterm -fg red -title "localhost" -e 'echo;echo "Starting local session ..."; echo;echo;cd /tmp/SystemWebotsRobotino3Navigation.deployment; pwd; bash --login -i -e start-PC1.sh start' &
	echo $! >> $PID_FILE
...
stop)
	...
	xterm -title "stopping localhost" -e 'echo;echo "Starting local session ..."; echo;echo;cd /tmp/SystemWebotsRobotino3Navigation.deployment; pwd; bash --login -i -e start-PC1.sh stop'
	echo $! >> $PID_FILE
